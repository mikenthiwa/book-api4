...
class Books(db.Model):
    ...
    def get_one(book_id):
        ...
        if book is None:
            return jsonify({"msg": "book is not available"})

        return jsonify({"book id": book.book_id, "book_title": book.book_title,
                        "book_author": book.book_author, "book_copies": book.book_copies})
    ...
    def modify_book_title(book_id, title):
        ...
        if title:
            ...
            db.session.commit()
            return jsonify({"msg": 'book title modified'})
    ...
    def modify_book_author(book_id, author):
        ...
        db.session.commit()
        return jsonify({"msg": 'book author modified'})
...
class Users(db.Model):
    ...
    def create_user(cls, username, email, password, admin=False):
        ...
        new_user = cls(public_id=str(uuid.uuid4()), username=username,
                       email=email, password=hashed_password, admin=admin)
        db.session.add(new_user)
        db.session.commit()
        return jsonify({"msg": "user crested"})
    ...
    def get_user(public_id):
        ...
        return jsonify({"username": user.username, "public_id": user.id, "admin": user.admin, "password": user.password})
